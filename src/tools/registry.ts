...
import {
	...
	heartbeatTool,
} from "./heartbeat/heartbeat-tool.js";
import {
...
import { QQ_SEND_DESCRIPTION, qqSendTool } from "./qq-send.js";
import {
	exaSearch,
	langSearch,
	perplexitySearch,
...
export function getToolRegistry(model: string): RegisteredTool[] {
	const tools: RegisteredTool[] = [
		{
			...
			compactDescription:
				"Financial statements and metrics. Handles multi-company/multi-metric queries in one call.",
		...
		},
		{
			...
			compactDescription:
				"Stock/crypto prices, company news, and insider trades. Handles multi-asset queries in one call.",
			concurrencySafe: true,
		},
		{
			name: "read_filings",
			...
			compactDescription:
				"SEC filings (10-K, 10-Q, 8-K). Extracts and summarizes specific filing sections.",
			concurrencySafe: true,
		},
		{
			name: "stock_screener",
			...
			compactDescription:
				"Screen stocks by financial criteria (P/E, growth, margins, etc.).",
			concurrencySafe: true,
		},
		{
			name: "web_fetch",
		...
		},
		{
			...
			compactDescription:
				"JavaScript-rendered pages and interactive navigation. Actions: navigate, snapshot, act, read, close.",
			concurrencySafe: true,
		},
		{
			name: "read_file",
		...
		},
		{
			...
			compactDescription:
				"Edit a file by replacing text. Requires user approval.",
			concurrencySafe: false,
		},
		{
			name: "heartbeat",
			...
			compactDescription:
				"View or update the periodic heartbeat checklist (.dexter/HEARTBEAT.md).",
			concurrencySafe: true,
		},
		{
			name: "cron",
			...
			compactDescription:
				"Manage scheduled cron jobs (create, list, update, delete).",
			concurrencySafe: true,
		},
		{
			name: "memory_search",
			...
			compactDescription:
				"Search persistent memory and past conversations for stored facts and preferences.",
			concurrencySafe: true,
		},
		{
			name: "memory_get",
		...
		},
		{
			name: "qq_send",
			tool: qqSendTool,
			description: QQ_SEND_DESCRIPTION,
			compactDescription:
				"Send a message via QQ Bot. Use for reports, alerts, or notifications to the user.",
			concurrencySafe: true,
		},
	];
	...
	if (allWebSearchProviders.length > 0) {
		const preferred = getSetting<SearchProviderId | undefined>(
	...
	if (availableSkills.length > 0) {
		tools.push({
			...
			compactDescription:
				"Invoke a specialized skill workflow (e.g., DCF valuation).",
			concurrencySafe: false,
		});
	}
...
export function getToolConcurrencyMap(model: string): Map<string, boolean> {
	return new Map(
		getToolRegistry(model).map((t) => [t.name, t.concurrencySafe]),
	);
}
